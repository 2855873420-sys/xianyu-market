...
function updateStats() {
    const stats = [document.getElementById('statTotal'), document.getElementById('statAvgPrice'), document.getElementById('statCategories'), document.getElementById('statRecent')];
    if (!stats[0]) return;
    const products = PRODUCTS;
    stats[0].textContent = products.length;
    if (products.length > 0) {
        const avg = products.reduce((s, p) => s + p.price, 0) / products.length;
        stats[1].textContent = '¥' + Math.round(avg).toLocaleString();
    } else { stats[1].textContent = '¥0'; }
    const categories = new Set(products.map(p => p.category));
    stats[2].textContent = categories.size;
    const now = new Date();
    const monthAgo = new Date(now.getFullYear(), now.getMonth(), 1);
    stats[3].textContent = products.filter(p => new Date(p.date) >= monthAgo).length;
}
...
function renderAdminTable() {
    ...
    if (search) {
        products = products.filter(p => p.title.toLowerCase().includes(search) || p.seller.toLowerCase().includes(search));
    }
    if (category !== 'all') {
    ...
    tbody.innerHTML = products.map(p => `
        <tr>
            <td>${p.id}</td><td class="td-icon">${p.image}</td>
            ...
            <td>${p.condition}</td><td>${escapeHtml(p.seller)}</td><td>${p.date}</td>
            <td class="td-actions">
...
async function saveAdminProduct(event) {
    ...
    if (editId) {
        ...
        if (idx !== -1) {
            PRODUCTS[idx] = { ...PRODUCTS[idx], title, category, price, condition, image: image || PRODUCTS[idx].image, desc: desc || '卖家很懒，什么都没有写~', seller, contact: contact || '未提供' };
        }
    } else {
        PRODUCTS.unshift({
            id: getNextId(PRODUCTS), title, category, price, condition,
            image: image || '📦', desc: desc || '卖家很懒，什么都没有写~',
            seller, contact: contact || '未提供', date: new Date().toISOString().split('T')[0]
        });
    }
    ...
    renderAdminTable();
    updateStats();
    showToast(saved ? '保存成功，已同步云端 ✅' : '保存成功（云端同步失败）⚠️');
}
...
function showToast(message) {
    ...
    toast.style.cssText = 'position:fixed;bottom:30px;left:50%;transform:translateX(-50%);background:#2d3436;color:white;padding:12px 24px;border-radius:24px;font-size:14px;font-weight:600;z-index:9999;animation:toastIn 0.3s ease,toastOut 0.3s ease 2s forwards;white-space:nowrap;';
